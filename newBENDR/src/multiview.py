...
class TimeClassifier(nn.Module):
    def __init__(self, in_features, num_classes, pool='adapt_avg', n_layers=1, orig_channels=9, time_length=33):
        ...
        self.channelreduction = nn.Linear(
            in_features=orig_channels, out_features=1)
    ...
    def __init__(self, in_features, output_dim, n_layers=1):
        ...
        if n_layers == 1:
            self.projector = nn.Linear(
                in_features=in_features, out_features=output_dim)
        elif n_layers == 2:
...
def conv1D_out_shape(input_shape, kernel, stride, padding):
    kernel, stride, padding = np.array(
        kernel), np.array(stride), np.array(padding)
    shape = input_shape
...
class Wave2Vec(nn.Module):
    def __init__(self,
        ...
        self.convblocks = nn.Sequential(*[wave2vecblock(channels_in=in_channels[i], channels_out=out_channels[i],
                                        kernel=width[i], stride=width[i], norm=norm, dropout=do) for i in range(nlayers)])
        self.readout = nn.Conv1d(
            out_dim, out_dim, 1) if readout_layer else nn.Identity()
        self.out_shape = conv1D_out_shape(
            input_shape, width, width, [w//2 for w in width])
...
class Multiview(nn.Module):
    def __init__(self,
        ...
        if projection_head:
            if not loss == 'time_loss':
                ...
                self.projector = TimeProjector(
                    in_features=out_dim, output_dim=embedding_dim, n_layers=n_layers_proj)
    ...
    def update_classifier(self, num_classes, orig_channels, seed=None):
        ...
        self.classifier = TimeClassifier(
            in_features=self.out_dim, num_classes=num_classes, pool='adapt_avg', orig_channels=orig_channels)
...
class GNNMultiview(nn.Module):
    ...
    def forward(self, x, classify=False):
        ...
        view_id = torch.arange(b).unsqueeze(
            1).repeat(1, ch).view(-1).to(x.device)
        message_from = torch.arange(
            b*ch).unsqueeze(1).repeat(1, (ch-1)).view(-1).to(x.device)
        message_to = torch.arange(
            b*ch).view(b, ch).unsqueeze(1).repeat(1, ch, 1)
        ...
        for message_net in self.message_nets:
            ...
            message = message_net(
                torch.cat([latents[message_from], latents[message_to]], dim=-1))/(ch-1)
            # Sum messages
    ...
    def train_step(self, x, loss_fn, device):
        ...
        # randomly select ch_size channels per input
        view_1_idx = random_channels[:ch_size]
        # take the remaining as the second view
        view_2_idx = random_channels[ch_size:]
...
def finetune(model,
    ...
    if early_stopping_criterion is not None:
        early_stopping = EarlyStopping(
            patience=7, verbose=True, path=f'{backup_path}/finetuned_model.pt')
    for epoch in range(epochs):
        epoch_loss = 0
        model.train()
        for i, data in enumerate(dataloader):
        ...
        if test_loader is not None:
            test_acc, test_prec, test_rec, test_f = evaluate_classifier(
        ...
        if early_stopping_criterion is not None:
            ...
            if early_stopping.early_stop:
                ...
                model.load_state_dict(torch.load(
                    f'{backup_path}/finetuned_model.pt'))
...
def load_model(pretraining_setup, device, channels, time_length, num_classes, model_args):
    ...
    if pretraining_setup == 'MPNN':
        model = GNNMultiview(channels=1, time_length=time_length,
                             num_classes=num_classes, **vars(model_args)).to(device)
    elif pretraining_setup == 'nonMPNN':
        model = Multiview(channels=1, orig_channels=6, time_length=time_length,
                          num_classes=num_classes, **vars(model_args)).to(device)
    elif pretraining_setup == 'None':
        model = Multiview(channels=channels, orig_channels=channels, time_length=time_length,
                          num_classes=num_classes, **vars(model_args)).to(device)
